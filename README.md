...
- 1. El frontend esté ejecutándose en el puerto por defecto de Vscode, el **5500**,  en la dirección IP loopback 127.0.0.1. (es decir en `localhost:5500`).
- 2. la base de datos esté exponiéndose en el puerto por defecto de mysql workbench, **3306** de la mencionada dirección.
- 3. El backend de springboot sea accesible a través del puerto por defecto **8080**.
...
2. Cada rombo de fondo **amarillo** es una decisión que se hará dentro del **back-end** de springboot, dado que requiere hacer consultas a la BBDD y contiene datos sensibles.
3. Los rombos de fondo azul se decidirán en el front-end en tanto que sus decisiones no requieran consultar información personal en la base de datos y no requerirán, por lo tanto, de uso del back-end (no se explicarán en este repositorio).
4. El paréntesis que incluye la extensión de una URL debajo de cada rectángulo naranja es cada página de Netflix cuyo comportamiento y, en menor medida, aspecto, se ha intentado replicar en el archivo html del rectángulo naranja que le es contiguo. Por ejemplo, el archivo html `pas2A_infoBenvinguda.html` de mi proyecto es una réplica de la página especificada en el paréntesis `netflix.com/signup/registration` y el usuario llegará a ella a través del proceso de registro gracias a la aplicación de una lógica de back-end similar a la que usa Netflix. 


>⚠️ **NOTA**: es muy importante hacer notar que las consultas de datos personales a la base de datos -correos y contraseñas- no se deben hacer desde el front-end, porque los archivos del front-end son públicos para el usuario y podrían exponer las bases de datos (y los datos de los usuarios) a vulnerabilidades.
...
Cuando un usuario introduce su correo en el formulario de registro de la landing page y le da al botón de registro se va a mandar el correo desde el front-end a un endpoint de la API REST del backend -en este caso, el endpoint está en la línea 171, que es esta URL: `'http://localhost:8080/api/usuariExisteix'`-. La información viajaár a través del _cuerpo_ o body de la solicitud POST mediante formato JSON: en este caso particular, si el correo introducido fuese `asd@ijk.com` entonces el mensaje transmitido sería un objeto javascript que tendría esta forma: `{"email":"asd@ijk.com"}`:
...
Una de las cosas que implementa Springboot es la posibilidad de mapear un objeto de una clase de Java con la línea de una tabla mysql mediante el uso de la anotación `@Entity`. Es lo que llaman ORM o "Object Relational Mapping". Vamos a comparar el DDL[^3] de la tabla "usuari" de mySQL con la clase anotada con `@Entity` que mapea a esa tabla, para verlo más claro.
