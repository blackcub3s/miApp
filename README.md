...
En este repositorio muestro la parte inicial de un proceso de registro e iniciar sesión de una aplicación web que estoy haciendo. Con él pretendo mostrar al lector una guía generalista de como programar y ejecutar un proyecto de backend con springboot (framework de backend de java).
...
La gracia de la aplicación web que contiene este repositorio es que mediante el back-end se ha intentado (y, en mi opinión, conseguido) hacer una réplica de la lógica de negocio que sigue Netflix cuando un usuario se registra en su portal web usando el formulario que la landimg page tiene para registrar el correo. Cuando este se registra en Netflix o en nuestra web, va a entrar en la antesala de un sistema preparado para brindarle, con la máxima sencillez posible, la inscripción a un servicio por suscripción. Cuando el usuario introduce su e-mail en ese formulario, el sistema evalúa cuál es el estado actual del usuario en nuestra base de datos: en función de ello el sistema le mostrará a éste un camino u otro. Si el usuario está registrado o no lo está, o bien si al estar registrado está pagando por servicios o no, generarán que el back-end redirija al usuario a distintas páginas de acuerdo a lo que se especifica en el [diagrama de flujo](#Diagrama-de-flujo) que mostraremos en el siguiente apartado. 

Además, la comunicación entre front-end y back-end la he realizado en todo caso mediante API (usando la interfaz nativa del navegador fetch-API en el front-end para que este pueda "consumir" los endpoints que nos proporciona la REST-API del backend). En todo caso se ha seguido en la medida de lo posible los principios REST (véase nota al pie[^1]). Así pues, debe quedar claro que el back-end y el front-end son dos proyectos separados que estan en comunicación entre ellos y que el back-end, además, se comunicará con la base de datos que será un tercer proyecto. la ventaja de hacer el proyecto por separado es que se podría reescribir, por ejemplo, el proyecto front-end en otra tecnología basada en componentes (por ejemplo Angular, React o Vue) sin tener que modificar el back-end, o que incluso podría usarse este backend para permitir que sus endpoints sean consumidos por otras aplicaciones además del front-end: por ejemplo, una aplicación en android.

# 1. Objetivo

Por un lado, mostrar una pequeña porción del software de un proyecto mio para demostrar que he adquirido un conocimiento generalista del framework Springboot; y, por el otro, permitir que cualquier persona que quiera empezar a desarrollar back-end con este framework de Java tenga una guía para poder a empezar a dar sus primeros pasos.

Para enseñar como funciona springboot voy a describir de forma pormenorizada la parte del árbol de decisión o diagrama de flujo que está marcada en color rosa en el apartado [diagrama de flujo](#Diagrama-de-flujo), explicando a grandes rasgos como se estructura un proyecto en springboot en sus distintas partes (modelo, repositorio, servicio y controlador). Además, también demostraré como podemos utilizar este framework para traducir el código en resultados tangibles y ordenados para la lógica de negocio. 
...
-  **[el back-end](/APP%20WEB/__springboot__produccio__/)**: se propone ejecturarlo mediante el editor de Java `IntelliJ idea`. Para hacerlo hay que buscar el archivo AppApplication.java, donde se situa la función main que en ser llamada mediante "ctrl + shift + F10" activará todo el proyecto.
...
- 1. el frontend corra en el puerto por defecto de vscode, el **5500**,  en la dirección IP loopback 127.0.0.1. (es decir en `localhost:5500`).
- 2. la base de datos corra en el puerto por defecto de mysql workbench, **3306** de la mencionada dirección.
- 3. El backend de springboot esté ejecutándose en el puerto por defecto **8080**.
